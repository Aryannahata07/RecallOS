...
export interface FSRSCard {
  ...
  stability: number;       // In days, representing time to reach 90% retrievability
  last_reviewed_at: Date | null;
  next_review_due: Date;
  reps: number;            // Total repetitions
  lapses: number;          // Total lapses (times forgotten)
}
...
export class FSRSScheduler {
   ...
   */
  public createInitialCard(rating: Rating, now: Date): { card: FSRSCard; interval: number } {
   ...
   */
  public reviewCard(card: FSRSCard, rating: Rating, now: Date): { card: FSRSCard; interval: number } {
    if (!card.last_reviewed_at) {
      return this.createInitialCard(rating, now);
    }

    const elapsedDays = Math.max(
      0.1 / 24, // Minimum 6 minutes
    ...
    if (rating === Rating.Again) {
      ...
      let ratingBonus = 1.0;
      if (rating === Rating.Hard) ratingBonus = this.w[15];
      if (rating === Rating.Easy) ratingBonus = this.w[16];
